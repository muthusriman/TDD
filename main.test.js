...
beforeEach(() => {
  ...
  ({ updateGroupTable } = require("./main.js"));
  jest.resetModules();
  localStorage.clear();
});
...
describe("User Management System", () => {
  ...
  test("should verify the presence of users tab", () => {
    ...
    expect(users.disabled).toBeFalsy();
  });

  test('should verify the presence of "users" text in the tab', () => {
    const userstxt = document.querySelector("#users-tab span");
  ...
  test("should verify the presence of groups tab", () => {
    const groups = document.querySelector("#groups-tab");
    expect(groups).toBeTruthy();
    expect(groups.disabled).toBeFalsy();
  });

  test('should verify the presence of "groups" text in the tab', () => {
    const groupstxt = document.querySelector("#groups-tab span");
  ...
  test("should verify the presence of roles tab", () => {
    const roles = document.querySelector("#roles-tab");
    expect(roles).toBeTruthy();
    expect(roles.disabled).toBeFalsy();
  });

  test('should verify the presence of "roles" text in the tab', () => {
    const rolestxt = document.querySelector("#roles-tab span");
  ...
  test("should verify the presence of modal header", () => {
    ...
    const modalHeading = document.querySelector("#userManagement .modal-heading");
    const closeIcon = document.querySelector("#userManagement .fa-regular.fa-circle-xmark");
...
describe("Group Management System", () => {
  ...
  test("should verify the presence of menu-icon", () => {
    const menuicon = document.querySelector(".fas.fa-bars");
    expect(menuicon).not.toBeNull();
  });
  ...
  test("should verify the presence of groups tab", () => {
    const groups = document.querySelector(".tab[data-tab-id='groupManagement']");
    expect(groups).toBeTruthy();
  });

  test('should verify the presence of "groups" text in the tab', () => {
    const groupstxt = document.querySelector(".tab[data-tab-id='groupManagement'] span");
    expect(groupstxt.textContent).toBe("GROUPS");
  });
  ...
  test("should verify the presence of 'Groups List' heading", () => {
    const groupsListHeading = document.querySelector(".groups-list h2");
    expect(groupsListHeading).toBeTruthy();
    expect(groupsListHeading.textContent).toBe("Groups List");
  });

  test("should verify the presence of 'Create Group' button", () => {
    const createButton = document.querySelector("#createGroupBtn");
    expect(createButton).toBeTruthy();
    expect(createButton.textContent.trim()).toBe("CREATE GROUP");
  });

  test("should verify the presence of group modal", () => {
  ...
  test("should verify the presence of modal header", () => {
    ...
    const closeModal = document.querySelector("#closeGroupModal");
    const closeIcon = document.querySelector(".fa-regular.fa-circle-xmark");
    expect(modalHead).toBeTruthy();
    expect(modalHeading.textContent).toBe("CREATE GROUP");
    expect(closeModal).toBeTruthy();
    expect(closeIcon).not.toBeNull();
  });

  test("should verify the presence of modal alert message", () => {
    const alert = document.querySelector("#groupFormMessage");
    expect(alert).toBeTruthy();
  });

  test("should verify the presence of form fields in modal", () => {
  ...
  test("should verify the presence of 'Create Group' button in the modal", () => {
    const createGroupButton = document.querySelector("#createGroupForm button[type='submit']");
    expect(createGroupButton).toBeTruthy();
    expect(createGroupButton.textContent).toBe("Create Group");
  });
  ...
  test("should open modal on 'Create Group' button click", () => {
    const createButton = document.querySelector("#createGroupBtn");
    const modal = document.querySelector("#groupModal");
    createButton.click();
    expect(modal.classList.contains("hidden")).toBe(false);
  ...
  xtest("should show error message when group name is missing", async () => {
    document.querySelector("#groupName").value = "";
    document.querySelector("#createGroupForm button[type='submit']").click();

    const formMessage = document.querySelector("#groupFormMessage");
    expect(formMessage.innerHTML).toContain("Please fill in the group name.");

    await new Promise((resolve) => setTimeout(resolve, 2000));
    expect(formMessage.textContent).toBe("");
  });

  xtest("should successfully add a group and update localStorage", async () => {
    document.querySelector("#groupName").value = "Admins";

    document.querySelector("#createGroupForm button[type='submit']").click();

    const alertDiv = document.querySelector(".alert");
    expect(alertDiv.innerHTML).toContain("Group added successfully!");
    ...
    const newGroup = groups.find((group) => group.name === "Admins");
    expect(newGroup).toBeTruthy();
    expect(newGroup.name).toBe("Admins");
  });

  xtest("should display the new group in the group table", async () => {
    document.querySelector("#groupName").value = "Admins";
    ...
    const table = document.querySelector(".group-table tbody");
    const rows = table.querySelectorAll("tr");
    expect(rows.length).toBeGreaterThan(0);

    const cells = rows[0].querySelectorAll("td");
    expect(cells[0].textContent.trim()).toBe("Admins");
    // Further checks for users in the group and actions
  });

  xtest("should display Add Users and Remove Users buttons for each group", async () => {
    document.querySelector("#groupName").value = "Admins";
    ...
    const table = document.querySelector(".group-table tbody");
    const rows = table.querySelectorAll("tr");
    const addUserButton = rows[0].querySelector(".add-user-button");
    const removeUserButton = rows[0].querySelector(".remove-user-button");

    expect(addUserButton).toBeTruthy();
    expect(removeUserButton).toBeTruthy();
  });

  xtest("should verify group removal functionality", async () => {
    document.querySelector("#groupName").value = "Admins";
    ...
    const table = document.querySelector(".group-table tbody");
    const rows = table.querySelectorAll("tr");
    const removeGroupButton = rows[0].querySelector(".remove-group-button");

    removeGroupButton.click();

    const alertDiv = document.querySelector(".alert");
    expect(alertDiv.innerHTML).toContain("Group removed successfully!");
    await new Promise((resolve) => setTimeout(resolve, 2000));
  ...
  test("should toggle sidebar and main content on toggle button click", () => {
    const sidebar = document.querySelector(".sidebar");
    const mainContent = document.querySelector("#main");
    ...
    toggleButton.click(); // Toggle again to revert
    expect(sidebar.classList.contains("closed")).toBe(false);
    expect(mainContent.classList.contains("expanded")).toBe(false);
  });

  test("should open the modal when 'Create Group' button is clicked", () => {
    const modal = document.querySelector("#groupModal");
    const createGroupButton = document.querySelector("#createGroupBtn");

    createGroupButton.click();
    expect(modal.classList.contains("hidden")).toBe(false);
    expect(document.querySelector("#groupName").value).toBe('');
    expect(document.querySelector("#groupFormMessage").textContent).toBe('');
  });
  ...
  test("should handle form submission and create a new group", async () => {
    document.querySelector("#groupName").value = "Admins";

    document.querySelector("#createGroupForm").dispatchEvent(new Event('submit'));

    const formMessage = document.querySelector("#groupFormMessage");
    expect(formMessage.textContent).toBe("Group created successfully!");
    expect(formMessage.style.color).toBe("rgb(40, 167, 69)"); // Success color
  ...
  test("should show error if group name is empty", () => {
    document.querySelector("#groupName").value = "";

    document.querySelector("#createGroupForm").dispatchEvent(new Event('submit'));

    const formMessage = document.querySelector("#groupFormMessage");
    expect(formMessage.textContent.trim()).toBe("Group name is required.");
  });

  test("should show error if group name already exists", () => {
    document.querySelector("#groupName").value = "Admins";
    ...
    const formMessage = document.querySelector("#groupFormMessage");
    expect(formMessage.textContent.trim()).toBe("Group name already exists.");
  });

  test("should update the group table after creating a new group", async () => {
    document.querySelector("#groupName").value = "Managers";
    ...
    const table = document.querySelector(".group-table tbody");
    const rows = table.querySelectorAll("tr");
    const newRow = Array.from(rows).find(row => row.querySelector("td").textContent.trim() === "Managers");
    expect(newRow).toBeTruthy();
  });

  test("should verify group actions buttons are present in the table", async () => {
    document.querySelector("#groupName").value = "Managers";
    ...
    const table = document.querySelector(".group-table tbody");
    const rows = table.querySelectorAll("tr");
    const actionButtons = rows[0].querySelectorAll("button");
    expect(actionButtons.length).toBe(3);
    expect(actionButtons[0].textContent).toBe("Add Users");
    expect(actionButtons[1].textContent).toBe("Remove Users");
    expect(actionButtons[2].textContent).toBe("Delete Group");
  });

  xtest("should delete a group from the table", async () => {
    document.querySelector("#groupName").value = "TemporaryGroup";
    ...
    const table = document.querySelector(".group-table tbody");
    const rows = table.querySelectorAll("tr");
    const deleteButton = rows[0].querySelector(".delete-group-btn");
    deleteButton.click();

    const formMessage = document.querySelector("#groupFormMessage");
    expect(formMessage.textContent).toBe("Group deleted successfully!");
    expect(formMessage.style.color).toBe("rgb(40, 167, 69)"); // Success color

    await new Promise((resolve) => setTimeout(resolve, 2000));

    const groups = JSON.parse(localStorage.getItem("groups"));
    expect(groups).not.toContainEqual(expect.objectContaining({ name: "TemporaryGroup" }));

    const updatedRows = table.querySelectorAll("tr");
    expect(updatedRows.length).toBe(0); // If "TemporaryGroup" was the only group
  });
});
